...
CREATE INDEX idx_product_name_gin ON product USING GIN (name gin_trgm_ops);
--  index better than btree index EXPLAIN (ANALYZE,BUFFERS,VERBOSE) SELECT * FROM product WHERE name LIKE 'f%';
...
CREATE TABLE IF NOT EXISTS table_list (
   id INTEGER GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
   name VARCHAR(30) NOT NULL,  /* Event, Store, , Product ... (tables) */
...
CREATE TABLE IF NOT EXISTS brand (
  ...
  created_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMPTZ,
  FOREIGN KEY (category_id) REFERENCES lukup_category(id) ON DELETE CASCADE ON UPDATE SET NULL;
);
...
CREATE TABLE IF NOT EXISTS brand_categories (
  ...
  created_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMPTZ,
  FOREIGN KEY (id_brand) REFERENCES brand(id) ON DELETE CASCADE ON UPDATE SET NULL,
  FOREIGN KEY (id_category) REFERENCES lukup_category(id) ON DELETE CASCADE ON UPDATE SET NULL;
);
...
CREATE TABLE IF NOT EXISTS store_document_ownership (
  id INTEGER GENERATED ALWAYS AS IDENTITY NOT NULL PRIMARY KEY,
  id_user INTEGER NOT NULL,
  ...
  updated_at TIMESTAMPTZ,
  FOREIGN KEY (id_user) REFERENCES "user"(id) ON UPDATE CASCADE ON DELETE NO ACTION,
...
CREATE TABLE IF NOT EXISTS inventory (
    ...
    FOREIGN KEY (id_store) REFERENCES store(id) ON DELETE SET NULL ON UPDATE CASCADE
);
CREATE INDEX idx_storeid_inventory ON inventory(id_store); 
...
CREATE TABLE IF NOT EXISTS "order" (
  ...
  id_status INTEGER, --Placed,Pending Payment,Processing,Partially Shipped,Shipped,Out for Delivery,Delivered,Cancelled,Returned
...
CREATE TABLE  IF NOT EXISTS chat_room_media (
 ...
 media_url TEXT,
 message TEXT,
 media_type SMALLINT NOT NULL, -- image, video ... 

 FOREIGN KEY (id_chat_room) REFERENCES chat_room(id) ON UPDATE CASCADE ,
 FOREIGN KEY (id_poster) REFERENCES "user"(id) ON UPDATE CASCADE ON DELETE SET NULL
);
...
CREATE TABLE IF NOT EXISTS seller (
  id INTEGER GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
  id_user INTEGER, 
  bmsname VARCHAR(30) UNIQUE NOT NULL, -- byte me seller name
  count INTEGER,
  rating DECIMAL(3,1) NOT NULL,
...
CREATE TABLE IF NOT EXISTS group_order (
    ...
    id_store INTEGER,
    max_amount_of_people INTEGER DEFAULT 18,
    is_pay_for_everyone BOOLEAN DEFAULT FALSE,
    is_repeatable BOOLEAN DEFAULT FALSE,
  
    group_order_uuid UUID DEFAULT gen_random_uuid(),
    group_order_deadline TIMESTAMPTZ,
    order_note TEXT,
    ...
    FOREIGN KEY (id_store) REFERENCES store ON DELETE CASCADE ON UPDATE CASCADE
);

CREATE TABLE IF NOT EXISTS repeatable_order (
    id INTEGER GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    id_grouporder INTEGER,
    from_date TIMESTAMPTZ NOT NULL,
    interval INTEGER,
    end_on_date TIMESTAMPTZ,

    FOREIGN KEY (id_grouporder) REFERENCES group_order ON DELETE CASCADE ON UPDATE CASCADE
);
